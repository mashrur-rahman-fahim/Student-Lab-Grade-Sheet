...
namespace StudentLabGradeSheet
{
    public static class ModernTheme
    {
        ...
        public static void StyleButton(Button button, Color backgroundColor, Color textColor, bool isPrimary = false)
        {
            ...
            button.Padding = new Padding(8, 5, 8, 5); // Reduced padding for better text fit
            button.TextAlign = ContentAlignment.MiddleCenter; // Center text properly
            button.UseCompatibleTextRendering = false; // Better text rendering
            ...
            {
                button.Height = 40; // Consistent height for all buttons
                button.FlatAppearance.MouseOverBackColor = DarkenColor(backgroundColor, 20);
                button.FlatAppearance.MouseDownBackColor = DarkenColor(backgroundColor, 40);
            }
        ...
        public static void ApplyGradientBackground(Form form)
        {
            form.Paint += (sender, e) =>
            {
                // Check if the rectangle is valid before creating the brush
